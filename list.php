...
<body>
    ...
    <?php
        ...
        while($row = mysqli_fetch_assoc($result)){
            ...
            echo '<div class="col-md-4"><button class="btn ' . $button_class . '" ' . $button_data . ' onclick="handleButtonClick(\'' . $row['Exam_id'] . '\', \'' . $row['Status'] . '\', \'' . $_GET['username'] . '\')">' . $row['Exam_name'] . '</button></div>';
            if($row_count % 2 != 0) {
                echo '</div>';
            }
            $row_count++;
